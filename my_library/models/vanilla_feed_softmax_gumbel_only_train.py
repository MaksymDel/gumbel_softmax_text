...
@Model.register("vanilla_softmax_gumbel_train")
class VanillaSoftmaxGumbelTrain(Model):

    """
    Whenever we do self-feeding, we do it with softmax predicted vectors.
    We do self-feeding during the inference, but with argmax.
    We do (Gumbel) softmax only during training (with scheduled sampling).
    ...
    def forward(self,  # type: ignore
        ...
        for timestep in range(num_decoding_steps):
            ...
            if self._weights_calculation_function == 'softmax':
                ...
                raise ValueError("Wrong calculation fucntion. Should be either 'gumber' or 'softmax'.")
            ...
            if self.training: # if training, produce differentiable vector with (Gumbel) softmax
                embedded_output = torch.matmul(class_probabilities, self._target_embedder.weight)
            else: # if inference, we can use non-differentiable argmax
